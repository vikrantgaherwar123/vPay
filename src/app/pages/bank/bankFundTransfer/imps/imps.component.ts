...
import { AccState, BeneState } from '../bankFundTransfer.component';
...
export class ImpsComponent implements OnInit {
    ...
    this.apiService.sendToServer<ICore>('/api/virtualPay/Get_Account', paramObj, this).subscribe((data) => {
      ...
      if (data.code == 1 && data.msg.toUpperCase() == 'SUCCESS') {
        if (data.cursor1[0].ERROR) {
          ...
          this.accImpsTranArray = data.cursor1;
          // this.FromAccountNumber1 = new FormControl();
          // this.filteredStates = this.FromAccountNumber1.valueChanges
          //   .pipe(
          //     startWith(''),
          //     map(from => from ? this.filterStates(from) : this.accImpsTranArray.slice())
          //   );
          // this.showFundTransfer = true;
          this.DivshowImps = true;
  ...
  // filterStates(ACCOUNT_NUMBER: any) {
  //   return this.accImpsTranArray.filter(from =>
  //     from.ACCOUNT_NUMBER.toLowerCase().indexOf(ACCOUNT_NUMBER.toLowerCase()) === 0);
  // }
  // filterStatesTo(BENE_ACCOUNT_NUMBER: any) {
  //   return this.BeneAccountList.filter(toItem =>
  //     toItem.BENE_ACCOUNT_NUMBER.toLowerCase().indexOf(BENE_ACCOUNT_NUMBER.toLowerCase()) === 0);
  // }

  // onEnter(evt: any, fObj) {
  //   if (evt.source.selected) {
  //     this.FromAccountchange(fObj);
  //   }
  // }
  // onEnterTo(evt: any, fObj) {
  //   if (evt.source.selected) {
  //     this.ToAccountchange(fObj);
  //   }
  // }
  //BeneFiciary List 
  Get_BeneficieryList(PFlag) {
    ...
    this.apiService.sendToServer<ICore>('/api/virtualPay/Bene_List', uinput, this).subscribe(data => {
      ...
      if (data.code === 1 && data.msg.toUpperCase() == 'SUCCESS') {
        ...
        // this.ToAccountNumber1 = new FormControl();
        // this.filteredStatesTo = this.ToAccountNumber1.valueChanges
        //   .pipe(
        //     startWith(''),
        //     map(toItem => toItem ? this.filterStatesTo(toItem) : this.BeneAccountList.slice())
        //   );
      }
      else {
  ...
  saveCode(e): void { 
    ...
    if(e.target.id=='txtFromAccountNumber'){
      list = this.accImpsTranArray.filter(x => x.ACCOUNT_NUMBER === AcNo)[0];
  ...
  ImpsTranMethod() {
    this.ImpsModeTitle = 'Transaction';
    this.showImpsTrans = true;
    this.showImpsTranfer = false;
    this.btnBack = false;
  }
  ...
  ImpsMMIDMethod() {
    ...
    MessageBox.show(this.dialog, this.message, "");
    return false;
    // this.ImpsModeTitle = 'MMID Generation';
    // this.showImpsMMID = true;
    // this.showImpsTranfer = false;
    // this.btnBack = false;
  }
  ImpsTranP2AMethod() {
    this.showImpsTrans = false;
    this.impsTranTitle = 'IMPS Transaction P2A Transfer';
  ...
  FromAccountchange(obj) {
    ...
    this.FromBRANCH_NAME = obj.BANK_NAME;
    // }
  }
  ...
  ToAccountchange(obj) {
    ...
    // if (event.keyCode === 13 || event.button === 0) {
    this.ToAccountNumber = obj.BENE_ACCOUNT_NUMBER;
    this.ToAccountName = obj.BENE_NAME;
    this.ToAccountIFSC = obj.IFSC_CODE;
    this.ToBank_Name = obj.BANK_NAME;
    this.BENE_MST_ID = obj.BENE_MST_ID;
    // }
  }

  //fundTranfer Submit
  impsP2ATranferSubmit(tranMode) {
    // this.FromAccountNumber = this.FromAccountNumber1.value;
    // this.ToAccountNumber = this.ToAccountNumber1.value;
    ...
    if (this.ToAccountNumber == undefined || this.ToAccountNumber == null || this.ToAccountNumber.toString().trim() == "") {
    ...
    var paramObj = {
      ...
      Bene_Account: this.ToAccountNumber,
        ...
        if (data.Po_Error != null) {
          ...
          if (data.code === 1) {
            if (data.cursor1[0].RESPONSE_CODE == '00') {
              ...
              this.otpCntrl.otp = '';
              // this.message = 'IMPS Transaction ' + data.cursor1[0].RESPONSE_MESSAGE + ' .';
              // var msg = 'Your Referance ID is ' + this.CBS_TranID + ' .';
              // MessageBox.show(this.dialog, this.message, msg, "");
              if (this.impsTranTitle == "IMPS Transaction P2A Transfer") {
                this.message = 'Your IMPS Transaction P2A ';
              }
              // MessageBox.show(this.dialog, this.message, "");
